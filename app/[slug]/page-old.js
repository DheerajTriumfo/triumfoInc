    ...
    replace: (domNode) => {
      ...
      switch (domNode.name) {
        case 'p':
          appendClass(
            attribs,
            `${isLight ? 'text-white' : 'text-gray-500'} ${
              isLight ? 'text-base' : 'text-lg'
        ...
        case 'a':
          appendClass(
            attribs,
            `${isLight ? 'text-white underline' : 'text-blue-600 hover:text-blue-800 underline'} transition-colors duration-200`
          );
          break;
        default:
          break;
...
export default async function Getlocation(props) {
  ...
  try {
    const res = await fetch(`${apiBase}/${encodeURIComponent(slug)}`, { cache: 'no-store' });
    if (res.ok) {
  ...
  if (!pageData) {
    ...
    try {
      const res = await fetch(`${apiBase}/rental/booth-size/${encodeURIComponent(rentalurl)}`, { cache: 'no-store' });
      if (res.ok) {
        const json = await res.json();
        pageData = json?.data || null;
  ...
  const rentalData = Array.isArray(pageData)
    ? pageData
    : Array.isArray(pageData?.rental_data)
    ? pageData.rental_data
    : [];

  const imagedata = Array.isArray(pageData?.imagedata) ? pageData.imagedata : [];

  const rentalDataWithImages = rentalData.map((booth) => {
    const imageObj = imagedata.find((img) => img.rentalid === booth.id);
    return {
      ...booth,
      image: imageObj ? imageObj.full_url : null,
    };
  });
  ...
  return (
    <>
            ...
            <div className="max-w-3xl mx-auto">
              <div className="text-center">
                ...
                {heroDescriptionContent ? (
                  <div className="flex flex-col items-center gap-2 text-white">{heroDescriptionContent}</div>
                ) : (
                  <p className="text-white font-normal text-base">{headingDesc}</p>
                )}
                <div className="mt-8">
                  <Link
                    href={is10x10 ? '/get-free-3d-design/' : '/get-booth-quotation/'}
                    className="px-7 py-3 bg-custom rounded-md border-2 border-custom text-white hover:bg-transparent hover:border-2 hover:border-white hover:text-white transition duration-300 text-xl"
                  >
                    {is10x10 ? 'Claim Your Free Design' : 'Get a Quote'}
                  </Link>
                </div>
              </div>
            </div>
      ...
      <section>
        <BoothGrid booths={rentalDataWithImages} apiBase={apiBase} parentSlug={slug} />
      </section>
          ...
          <div className="container mx-auto px-4">
            <div className="bg-gray-900 text-white rounded-xl p-6 md:p-10 mx-4 md:mx-auto max-w-7xl my-10">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                  <h2 className="text-xl md:text-4xl font-semibold mb-3 barlofamilty">Looking for something different?</h2>
        ...
        <div className="clientsection py-20">
          <div className="container mx-auto px-4">
            <div className="max-w-4xl mx-auto">
              ...
              {sectionBodyContent ? (
                <div className="mt-8 flex flex-col gap-2 text-left">{sectionBodyContent}</div>
              ) : (
                <p className="text-gray-500 text-lg mt-8 text-center">
                  For 25 years, Triumfo Inc. is offering comprehensive solutions from creative booth designing and robust booth building to stress-free delivery and dismantling of trade show booths and rentals.
                </p>
              )}
            </div>
          </div>
